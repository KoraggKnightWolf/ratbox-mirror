...
static adns_status cs_inaddr(vbuf *vb, const void *datap) {
  ...
  rb_inet_ntop(AF_INET, rr, ia, sizeof(ia));
  CSP_ADDSTR(ia);
  return adns_s_ok;
}
  ...
  case AF_INET6:
    {
       char ip[IP6STRLEN];
       CSP_ADDSTR("INET6 ");
       rb_inet_ntop(AF_INET6, rrp->addr.inet6.sin6_addr.s6_addr, ip, IP6STRLEN);
       CSP_ADDSTR(ip);
       break;
    }
#endif
...
static adns_status pa_ptr6_all(const parseinfo *pai, int dmstart, int max, void *datap, const char **expectdomain)
{
  ...
  if (!ap->len) {
    ...
    *pt = '\0';
    ...
    rb_inet_pton(AF_INET6, ip6, ap->addr.inet6.sin6_addr.s6_addr);
  }
