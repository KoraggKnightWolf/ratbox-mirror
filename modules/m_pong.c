...
DECLARE_MODULE_AV1(pong, NULL, NULL, pong_clist, NULL, NULL, "$Revision: 19256 $");

static int
ms_pong(struct Client *UNUSED(client_p), struct Client *source_p, int UNUSED(parc), const char *parv[])
{
	...
	if(IsServer(source_p) && !HasSentEob(source_p))
	{
		if(MyConnect(source_p))
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     ...
					     source_p->name,
					     (signed int) (ircd_currenttime - source_p->localClient->firsttime));
		SetEob(source_p);
		eob_count++;
	}
...
mr_pong(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	if(parc == 2 && !EmptyString(parv[1]))
	{
		...
		{
			u_int32_t incoming_ping = strtoul(parv[1], NULL, 16);
			if(incoming_ping)
			{
