...
DECLARE_MODULE_AV1(invite, NULL, NULL, invite_clist, NULL, NULL, "$Revision: 19256 $");
...
m_invite(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	{
		sendto_one(source_p, POP_QUEUE, form_str(RPL_INVITING), 
	...
	{
		sendto_one(target_p, POP_QUEUE, ":%s!%s@%s INVITE %s :%s", 
...
add_invite(struct Channel *chptr, struct Client *who)
{
	...
	if((int)ircd_dlink_list_length(&who->localClient->invited) >= 
	...
	ircd_dlinkAddAlloc(chptr, &who->localClient->invited);
}
