...
DECLARE_MODULE_AV1(close, NULL, NULL, close_clist, NULL, NULL, "$Revision: 19256 $");
...
mo_close(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	DLINK_FOREACH_SAFE(ptr, ptr_next, unknown_list.head)
	{
		...
		sendto_one(source_p, POP_QUEUE, form_str(RPL_CLOSING), me.name, source_p->name,
	...
	sendto_one(source_p, POP_QUEUE, form_str(RPL_CLOSEEND), me.name, source_p->name, closed);
	return 0;
}
