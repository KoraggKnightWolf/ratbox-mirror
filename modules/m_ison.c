...
DECLARE_MODULE_AV1(ison, NULL, NULL, ison_clist, NULL, NULL, "$Revision: 19295 $");
...
m_ison(struct Client * UNUSED(client_p), struct Client *source_p, int parc, const char *parv[])
{
	...
	*buf2 = '\0';
	...
	ircd_sprintf(buf, form_str(RPL_ISON), me.name, source_p->name);
	len = strlen(buf);
	...
	for (i = 1; i < parc; i++)
	{
		...
		for (nick = strtok_r(cs, " ", &p); nick; nick = strtok_r(NULL, " ", &p))
		{
