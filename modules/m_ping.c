...
DECLARE_MODULE_AV1(ping, NULL, NULL, ping_clist, NULL, NULL, "$Revision: 19256 $");
...
m_ping(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if(!EmptyString(destination) && !match(destination, me.name))
	{
		...
		sendto_one(source_p, POP_QUEUE, ":%s PONG %s :%s", me.name,
...
ms_ping(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	   ...
	   irccmp(destination, me.id))
	{
		...
		sendto_one(source_p, POP_QUEUE, ":%s PONG %s :%s", 
