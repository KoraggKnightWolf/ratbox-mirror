	      ...
	      unsigned long nocaps, const char *format, ...)
{
	...
	/* noone to send to.. */
	if(rb_rb_dlink_list_length(&serv_list) == 0)
		return;
		     ...
		     struct Channel *chptr, const char *pattern, ...)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, chptr->members.head)
	{
		...
		if(!MyClient(target_p))
		{
			...
			send_linebuf(target_p, &rb_linebuf_local, 0);
	}
			...
			int nocap, const char *pattern, ...)
{
	...
	RB_DLINK_FOREACH(ptr, global_serv_list.head)
	{
		...
		if(match(mask, target_p->name))
		{
			...
			if(has_id(target_p->from))
				send_linebuf(target_p->from, &rb_linebuf_id, 0);
			else
				send_linebuf(target_p->from, &rb_linebuf_name, 0);
		}
	}
...
kill_client_serv_butone(struct Client *one, struct Client *target_p, const char *pattern, ...)
{
	...
	RB_DLINK_FOREACH_SAFE(ptr, next_ptr, serv_list.head)
	{
		...
		if(has_id(client_p))
			send_linebuf(client_p, &rb_linebuf_id, 0);
		else
			send_linebuf(client_p, &rb_linebuf_name, 0);
	}
